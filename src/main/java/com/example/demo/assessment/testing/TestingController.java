...
public class TestingController {
    ...
    public ResponseEntity<Testing> createTesting(@RequestBody Testing testing) {
        try {
            Testing _testing = testingRepository
                    .save(new Testing(testing.getBasicInfoId(), testing.getAuxiliaryCheckNum(),
    ...
    public ResponseEntity<Testing> updateTesting(@PathVariable("id") long id, @RequestBody Testing testing) {
        ...
        if (testingData.isPresent()) {
            ...
            _testing.setBasicInfoId(testing.getBasicInfoId());
    ...
    @GetMapping("/testings/basicInfoId")
    public ResponseEntity<List<Testing>> findByBasicInfoId(@RequestParam() long basicInfoId) {
        try {
            List<Testing> testings = testingRepository.findByBasicInfoId(basicInfoId);
            if (testings.isEmpty()) {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
            return new ResponseEntity<>(testings, HttpStatus.OK);
